...
from apps.repo.models import Questions,Category,Answers,AnswersCollection
import base64
...
class QuestionsView(View):
    def get(self, request):
        ...
        questions_list = Questions.objects.filter()
    ...
    def get(self, request,id):
        ...
        if not result[1]:
            ...
            if question_collection.status:
                ...
                # UserLog.objects.create(question=question, operate=2,)
            else:
                question_collection.status=True
                # UserLog.objects.create(user=self.request.user, operate=1)
